<?php 
function atk_report_penggunaan_form(){
	...
	$form['reservasi'] = array(
			'#type' => 'fieldset',
			'#title' => t('Report Reservasi Material'),
			'#weight' => 1,
...
function atk_report_penggunaan_form_submit($form, &$form_state) {
	...
	drupal_goto('atk/online/report/penggunaan/all/' . $awal_reservasi. '/' . $akhir_reservasi);
}
function atk_report_penggunaan_all() {				// ============================= 1
	if(arg(5) && arg(6)){
		$awal 	= arg(5);
		$akhir 	= arg(6);
	}else{
	...
	$data = atk_report_penggunaan_data($awal,$akhir);
	$back = "<a href='" .base_path(). "atk/online/report/penggunaan'>[back]</a> | <a href='" .base_path(). "atk/online/report/penggunaan/toexcel/" .$awal. "/" .$akhir. "'>[to Excel]</a><br><br>";
	$output = theme('table', $data['judul'], $data['isi']);
	return $back.$output;
}

function atk_report_penggunaan_data($awal,$akhir) {
	...
	while($row = db_fetch_array($db_data)) {
		...
		if($row['statusApproval'] == 3){
			$info	= '<a href="' .base_path(). 'atk/online/reservation/info/' .$row['reservasiNo']. '/3/9"><input type="button" value="Send" /></a>';
			$idpl	= $req['lokasi']; $loka = atk_plant_data($idpl); $lokasi = $loka['description'];
			$isi[] 	= array(++$xyz, $row['reservasiNo'], $waktu, $stat, $lokasi, $fungsi['fungsi'], $req['fullname'], $row['materialCode'], $material, $row['requestQty'], $row['acceptQty'], $info);
		}
	}
	db_set_active();
...
function atk_report_penggunaan_toexcel(){		// ============================= 7
	if(arg(5) && arg(6)){
		$awal 	= arg(5);
		$akhir 	= arg(6);
	}else{
	...
	$workbook->send('report_belum_diambil.xls');
	$worksheet =& $workbook->addWorksheet('report_belum_diambil');
